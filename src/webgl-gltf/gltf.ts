...
const loadAnimation = (
  ...
  const channels = animation.channels.map((c) => {
    ...
    const time = readBufferFromFile(
      gltf,
      buffers,
      gltf.accessors![sampler.input]
    );
    const buffer = readBufferFromFile(
      gltf,
      buffers,
...
const loadMaterial = async (
  ...
  if (material.emissiveFactor) {
    emissiveFactor = vec3.fromValues(
...
const loadModel = async (gl: GLContext, uri: string) => {
  ...
  const materials = gltf.materials
    ? await Promise.all(
        gltf.materials.map(
          async (m) => await loadMaterial(gl, m, uri, gltf.images)
        )
      )
    : [];
  ...
  gltf.animations?.forEach(
    (anim) =>
      (animations[anim.name as string] = loadAnimation(gltf, anim, buffers))
  );

  const skins = gltf.skins
    ? gltf.skins.map((x) => {
        const bindTransforms = readBufferFromFile(
          gltf,
          buffers,
          gltf.accessors![x.inverseBindMatrices!]
        );
