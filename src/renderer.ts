...
import { Model, GLBuffer } from './gltf/types/model';
...
const renderModel = (model: Model, node: number, transform: mat4, shader: DefaultShader) => {
    const t = mat4.create();
    ...
    if (model.nodes[node].mesh !== undefined) {
        ...
        if (material) {
            applyTexture(material.baseColorTexture, 1, shader.baseColorTexture, shader.hasBaseColorTexture);
            applyTexture(material.roughnessTexture, 2, shader.roughnessTexture, shader.hasRoughnessTexture);
            applyTexture(material.emissiveTexture, 3, shader.emissiveTexture, shader.hasEmissiveTexture);
            applyTexture(material.normalTexture, 4, shader.normalTexture, shader.hasNormalTexture);
            applyTexture(material.occlusionTexture, 5, shader.occlusionTexture, shader.hasOcclusionTexture);
    ...
    model.nodes[node].children.forEach(c => {
        renderModel(model, c, transform, shader);
    });
};
